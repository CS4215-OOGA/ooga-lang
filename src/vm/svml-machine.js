...
var svml_scheduler_1 = require("./svml-scheduler");
var util_1 = require("./util");
var LDCI_VALUE_OFFSET = 1;
var LDCF32_VALUE_OFFSET = 1;
var LDCF64_VALUE_OFFSET = 1;
var LGCS_VALUE_OFFSET = 1;
var FUNC_MAX_STACK_SIZE_OFFSET = 0;
var FUNC_ENV_SIZE_OFFSET = 1;
var FUNC_NUM_ARGS_OFFSET = 2;
var FUNC_CODE_OFFSET = 3;
var INS_OPCODE_OFFSET = 0;
var BR_OFFSET = 1;
var LD_ST_INDEX_OFFSET = 1;
var LD_ST_ENV_OFFSET = 2;
var CALL_NUM_ARGS_OFFSET = 1;
var CALLT_NUM_ARGS_OFFSET = 1;
var CALLP_ID_OFFSET = 1;
var CALLP_NUM_ARGS_OFFSET = 2;
var CALLTP_ID_OFFSET = 1;
var CALLTP_NUM_ARGS_OFFSET = 2;
var CALLV_ID_OFFSET = 1;
var CALLV_NUM_ARGS_OFFSET = 2;
var CALLTV_ID_OFFSET = 1;
var CALLTV_NUM_ARGS_OFFSET = 2;
var NEWC_ADDR_OFFSET = 1;
var ADDR_FUNC_INDEX_OFFSET = 0;
var NEWENV_NUM_ARGS_OFFSET = 1;
var NEWCP_ID_OFFSET = 1;
var NEWCV_ID_OFFSET = 1;
// VIRTUAL MACHINE
...
var INTERNAL = vm_prelude_1.INTERNAL_FUNCTIONS;
var INTERNAL_OPCODE_SLOT = 1;
var INTERNAL_NUM_ARGS_SLOT = 2;
var INTERNAL_HAS_RETURN_VAL_SLOT = 3;
...
function show_registers(s, isShowExecuting) {
    if (isShowExecuting === void 0) { isShowExecuting = true; }
    var str = '';
    if (isShowExecuting) {
...
var DIV_ERROR = 1;
var TYPE_ERROR = 2;
var NUM_ARGS_ERROR = 3;
var CALL_NON_FUNCTION_ERROR = 4;
var ERROR_MSG_ARGS = [];
var STATE = NORMAL;
// general node layout
var TAG_SLOT = 0;
...
var ARRAY_TAG = -108;
var ARRAY_SIZE = 6;
var ARRAY_VALUE_SLOT = 4;
var ARRAY_SIZE_SLOT = 5;
// changes A, B
function NEW_ARRAY() {
    A = ARRAY_TAG;
    B = ARRAY_SIZE;
...
var UNDEFINED_TAG = -106;
var UNDEFINED_SIZE = 4;
...
var CLOSURE_TAG = -103;
var CLOSURE_SIZE = 7;
var CLOSURE_NORMAL_TYPE = 0;
// not necessary right now due to the way primitives are handled
// const CLOSURE_PRIMITIVE_TYPE = 1
var CLOSURE_INTERNAL_TYPE = 2;
var CLOSURE_TYPE_SLOT = 4;
var CLOSURE_FUNC_INDEX_SLOT = 5;
var CLOSURE_ENV_SLOT = 6;
// changes A, B, E, F
...
var RTS_FRAME_TAG = -104;
var RTS_FRAME_SIZE = 8;
var RTS_FRAME_PC_SLOT = 4;
var RTS_FRAME_ENV_SLOT = 5;
var RTS_FRAME_OS_SLOT = 6;
var RTS_FRAME_FUNC_INS_SLOT = 7;
...
var NORMAL_CALL = 0;
var TAIL_CALL = 1;
var PRIMITIVE_CALL = 2;
var PRIMITIVE_TAIL_CALL = 3;
var INTERNAL_CALL = 4;
var INTERNAL_TAIL_CALL = 5;
...
var scheduler = new svml_scheduler_1.RoundRobinScheduler();
var threads = new Map();
...
function RUN_THREAD() {
    var _a, _b;
    ;
    _a = scheduler.runThread(), currentThreadId = _a[0], TO = _a[1];
    _b = threads.get(currentThreadId), OS = _b[0], ENV = _b[1], PC = _b[2], P = _b[3], RTS = _b[4], TOP_RTS = _b[5];
}
...
var M = [];
M[opcodes_1.default.NOP] = function () {
    PC = PC + 1;
};
M[opcodes_1.default.LGCI] = function () {
...
M[opcodes_1.default.LGCF32] = function () {
...
M[opcodes_1.default.LGCF64] = function () {
...
M[opcodes_1.default.LGCB0] = function () {
    A = false;
    NEW_BOOL();
    A = RES;
    PUSH_OS();
...
M[opcodes_1.default.LGCB1] = function () {
    A = true;
    NEW_BOOL();
    A = RES;
    PUSH_OS();
...
M[opcodes_1.default.LGCU] = function () {
    NEW_UNDEFINED();
    A = RES;
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.LGCN] = function () {
    NEW_NULL();
    A = RES;
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.LGCS] = function () {
...
M[opcodes_1.default.POPG] = function () {
    POP_OS();
    PC = PC + 1;
};
// type check here as we need to know whether number or string
M[opcodes_1.default.ADDG] = function () {
    POP_OS();
    I = RES;
    POP_OS();
    G = RES;
    ...
    if (J) {
        ...
        ERROR_MSG_ARGS[1] = "".concat(node_kind(HEAP[G + TAG_SLOT]), " and ").concat(node_kind(HEAP[I + TAG_SLOT]));
        ERROR_MSG_ARGS[2] = '+';
        RUNNING = false;
    }
};
M[opcodes_1.default.SUBG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
    ...
    if (!G) {
        ...
        ERROR_MSG_ARGS[1] = "".concat(node_kind(HEAP[E + TAG_SLOT]), " and ").concat(node_kind(HEAP[D + TAG_SLOT]));
        ERROR_MSG_ARGS[2] = '-';
        RUNNING = false;
    }
};
M[opcodes_1.default.MULG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.DIVG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.MODG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.NEGG] = function () {
    POP_OS();
    D = RES;
    A = -HEAP[D + NUMBER_VALUE_SLOT];
    NEW_NUMBER();
    ...
    if (!G) {
        ...
        ERROR_MSG_ARGS[1] = "".concat(node_kind(HEAP[D + TAG_SLOT]));
        ERROR_MSG_ARGS[2] = '-';
        RUNNING = false;
    }
};
M[opcodes_1.default.NOTG] = function () {
    POP_OS();
    D = RES;
...
M[opcodes_1.default.LTG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.GTG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.LEG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.GEG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
...
M[opcodes_1.default.EQG] = function () {
    POP_OS();
    C = RES;
    POP_OS();
    D = RES;
...
M[opcodes_1.default.NEQG] = function () {
    POP_OS();
    C = RES;
    POP_OS();
    D = RES;
...
M[opcodes_1.default.NEWC] = function () {
...
M[opcodes_1.default.NEWA] = function () {
    NEW_ARRAY();
    A = RES;
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.LDLG] = function () {
    C = ENV;
    A = HEAP[C + HEAP[C + FIRST_CHILD_SLOT] + P[PC][LD_ST_INDEX_OFFSET]];
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.STLG] = function () {
    POP_OS();
    C = ENV;
    HEAP[C + HEAP[C + FIRST_CHILD_SLOT] + P[PC][LD_ST_INDEX_OFFSET]] = RES;
    PC = PC + 1;
};
M[opcodes_1.default.LDPG] = function () {
...
M[opcodes_1.default.STPG] = function () {
    POP_OS();
    B = P[PC][LD_ST_ENV_OFFSET]; // index of env to lookup
    C = ENV;
    for (; B > 0; B = B - 1) {
...
M[opcodes_1.default.LDAG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
    ...
    if (!G) {
        ...
        ERROR_MSG_ARGS[1] = "".concat(node_kind(HEAP[E + TAG_SLOT]));
...
M[opcodes_1.default.STAG] = function () {
    POP_OS();
    D = RES;
    POP_OS();
    E = RES;
    ...
    if (!G) {
        STATE = TYPE_ERROR;
        ERROR_MSG_ARGS[0] = 'array';
        ERROR_MSG_ARGS[1] = "".concat(node_kind(HEAP[F + TAG_SLOT]));
        ERROR_MSG_ARGS[2] = 'array access';
        RUNNING = false;
        return;
    }
...
M[opcodes_1.default.BRT] = function () {
    POP_OS();
...
M[opcodes_1.default.BRF] = function () {
    POP_OS();
    A = HEAP[RES + BOOL_VALUE_SLOT];
    if (!A) {
        PC = PC + P[PC][BR_OFFSET];
...
M[opcodes_1.default.BR] = function () {
    PC = PC + P[PC][BR_OFFSET];
};
M[opcodes_1.default.CALL] = function () {
...
M[opcodes_1.default.CALLT] = function () {
...
M[opcodes_1.default.CALLP] = function () {
...
M[opcodes_1.default.CALLTP] = function () {
...
M[opcodes_1.default.CALLV] = function () {
...
M[opcodes_1.default.CALLTV] = function () {
...
M[opcodes_1.default.RETG] = function () {
    POP_RTS();
    H = RES;
...
M[opcodes_1.default.DUP] = function () {
    POP_OS();
    A = RES;
    PUSH_OS();
    PUSH_OS();
...
M[opcodes_1.default.NEWENV] = function () {
    B = ENV;
...
M[opcodes_1.default.POPENV] = function () {
...
M[opcodes_1.default.ARRAY_LEN] = function () {
    POP_OS();
    D = RES;
...
M[opcodes_1.default.DISPLAY] = function () {
    POP_OS();
    C = RES;
    POP_OS();
    D = RES;
...
M[opcodes_1.default.DRAW_DATA] = function () {
    POP_OS();
    externalFunctions.get(opcodes_1.default.DRAW_DATA).apply(void 0, convertToJsFormat(RES));
    A = RES;
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.ERROR] = function () {
    POP_OS();
    C = RES;
    POP_OS();
    D = RES;
...
M[opcodes_1.default.IS_ARRAY] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_BOOL] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_FUNC] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_NULL] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_NUMBER] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_STRING] = function () {
    POP_OS();
...
M[opcodes_1.default.IS_UNDEFINED] = function () {
    POP_OS();
...
M[opcodes_1.default.MATH_HYPOT] = function () {
    POP_OS();
    A = Math.hypot.apply(Math, convertToJsFormat(RES));
    NEW_NUMBER();
    A = RES;
    PUSH_OS();
    PC = PC + 1;
};
M[opcodes_1.default.STRINGIFY] = function () {
    POP_OS();
...
M[opcodes_1.default.PROMPT] = function () {
    POP_OS();
...
M[opcodes_1.default.DISPLAY_LIST] = function () {
    POP_OS();
    C = RES;
    POP_OS();
    D = RES;
...
M[opcodes_1.default.ARITY] = function () {
    POP_OS();
    D = RES;
...
M[opcodes_1.default.EXECUTE] = function () {
    I = G;
    ...
    ;
    OS = G // restore state
    [0], ENV = G // restore state
    [1], PC = G // restore state
    [2], P = G // restore state
    [3], RTS = G // restore state
    [4], TOP_RTS = G // restore state
    [5]; // restore state
};
M[opcodes_1.default.TEST_AND_SET] = function () {
    POP_OS();
    D = RES; // array
...
M[opcodes_1.default.CLEAR] = function () {
    POP_OS();
    D = RES; // array
    E = HEAP[D + TAG_SLOT] === ARRAY_TAG;
    if (!E) {
...
function run() {
    var MAX_TIME = constants_1.JSSLANG_PROPERTIES.maxExecTime;
...
function getErrorType() {
    switch (STATE) {
        ...
        case TYPE_ERROR:
            ...
            // 1: received types
            // 2: operator
            return "Expected ".concat(ERROR_MSG_ARGS[0], ", got ").concat(ERROR_MSG_ARGS[1], " for ").concat(ERROR_MSG_ARGS[2], ".");
        case NUM_ARGS_ERROR:
            return "Expected ".concat(ERROR_MSG_ARGS[0], " arguments, but got ").concat(ERROR_MSG_ARGS[1], ".");
        case CALL_NON_FUNCTION_ERROR:
            return "calling non-function value ".concat(ERROR_MSG_ARGS[0], ".");
        default:
...
function convertToJsFormat(node, refs) {
    ...
    var kind = node_kind(HEAP[node + TAG_SLOT]);
    switch (kind) {
        case 'undefined':
            return undefined;
        case 'null':
        ...
        case 'array': {
            ...
            var arr = HEAP[node + BOXED_VALUE_SLOT];
            var res = [];
            refs.set(node, res);
...
function runWithProgram(p, context) {
    ...
    var externals = context.nativeStorage.builtins;
    if (externals.size > 0) {
        vm_prelude_1.EXTERNAL_PRIMITIVES.forEach(function (func) { return extractExternalBuiltin(func, externals); });
    }
    return run();
}
...
function addPrimitiveOpCodeHandlers() {
    ...
    vm_prelude_1.NULLARY_PRIMITIVES.forEach(function (func) {
        if (func[2])
            addNullaryHandler(func[1], func[2]);
    });
    vm_prelude_1.UNARY_PRIMITIVES.forEach(function (func) {
        if (func[2])
            addUnaryHandler(func[1], func[2]);
    });
    vm_prelude_1.BINARY_PRIMITIVES.concat([
        ['', opcodes_1.default.MATH_MAX, Math.max], // only want the handler
